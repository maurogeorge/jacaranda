  ...
  describe "with meta_validation" do
    ...
    describe "scopes" do
      ...
      it "have the scopes" do
        expect(PostWithMetaValidation.respond_to?(:publisheds)).to be_true
      end

      it "create scoped scopes" do
        expect(PostWithMetaValidationScoped.respond_to?(:statuses_publisheds)).to be_true
      end

      it "is a relation" do
        expect(PostWithMetaValidation.publisheds).to be_kind_of(ActiveRecord::Relation)
      end
      ...
      describe "returned values" do
        ...
        it "return correct values" do
          expect(PostWithMetaValidation.publisheds).to include(post_published)
        end

        it "not return incorrect values" do
          expect(PostWithMetaValidation.publisheds).to_not include(post_not_published)
        end
      end
    end
